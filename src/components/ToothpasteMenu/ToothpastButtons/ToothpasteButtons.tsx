import React, { useLayoutEffect, useRef, useState, ComponentProps } from 'react';
...
export default function ToothpasteButtons({ children }: ComponentProps<any>) {
  ...
  function getCurrentWidths() {
    ...
    return {
      outerWidth: (outerContainerRef.current as HTMLElement).getBoundingClientRect().width,
      innerWidth: (innerContainerRef.current as unknown as HTMLElement).getBoundingClientRect().width
    };
  }
  ...
  function countItemsToRemove() {
    ...
    const widthsArray = Array.from(widthsMap.values()).map(item => item.width);
